...
label_video = font.render('Изображение с камеры', True, BLACK)
label_video_rect = label_video.get_rect()
label_video_rect.center = 208, crane_side_view_Y - 15
label_above_view = font.render('Вид сверху', True, BLACK)
label_above_view_rect = label_above_view.get_rect()
label_above_view_rect.center = 650, crane_side_view_Y - 15
...
# флаги для перемещения крюка и поворота стрелы
...
while running:
    ...
    arrow_up_view_rotated = pygame.transform.rotozoom(arrow_up_view, arrow_angle, 0.5)
    arrow_up_view_rotated.convert()
    arrow_up_view_rotated_rect = arrow_up_view_rotated.get_rect()
    arrow_up_view_rotated_rect.center = 650, 460
    # отрисовка объектов
    screen.fill(WHITE)
    ...
    screen.blit(label_video, label_video_rect)
    ...
    pygame.draw.circle(screen, BLACK, arrow_up_view_rotated_rect.center, arrow_up_view.get_width() // 4, 1)

    video = pygame.image.frombuffer(image.tobytes(), image.shape[1::-1], "BGR")
    video=pygame.transform.rotozoom(video, 0, 0.65)

    screen.blit(video, (0,arrow_up_view_rotated_rect.centery-arrow_up_view.get_width() // 4))
